...
var race_cars: Array = [
	# idx0: { "icon" => Texture2D, "stats" => Stats, "car" => RaceCar, "position" => Node3D },
	# idx1: { "icon" => Texture2D, "stats" => Stats, "car" => RaceCar, "position" => Node3D },
]
...
func _circuit_ready():
	...
	self.road_start.connect("lights_out", Callable(self, "_on_lights_out"), CONNECT_ONE_SHOT)
	match Global.game_play_mode:
		...
		_:
			print("game play mode '", Global.game_play_mode, "' not implemented, yet")
			return

	var my_race_car = self.race_cars[self.race_cars_idx[self.my_race_car_id]].car
	my_race_car.connect("camera_position_changed", Callable(self.chase_camera, "_on_camera_position_changed"))

	self.zoom_camera.position = my_race_car.init_camera_position()
	self.zoom_camera.connect("camera_position_set", Callable(self, "_on_zoom_camera_position_set"), CONNECT_ONE_SHOT)
...
func _on_race_car_entered(car: RaceCar, road_idx: int):
	...
	# 	print(car.get_instance_id(), " ", road_idx, " V=", car._velocity.length())
	...
	if car.get_instance_id() == self.my_race_car_id:
		...
		if stats.lap_idx() == self.laps_count:
			...
			get_tree().create_timer(1.0).timeout.connect(_on_finish_race)
		else:
...
func _on_finish_race():
	...
	# [
	# 	{ "icon" => Texture2D, "label" => String, "total" => float }
	# ]
	var results: Array = []
	for rc in self.race_cars:
